			...
			HTTPTask task = new HTTPTask(new Gson().toJson(callbackdata),
					"authenticate", new WebServiceCallBack() {
						...
						public void onRequestComplete(Object responseData,
							...
							try {																
								//CallBack objClass = new Gson().fromJson(responseData.toString(),CallBack.class);
								...
								ISOSPlugin.CallBackLoginService(data);
							} catch (Exception e) {
								e.printStackTrace();
			...
			HTTPTask task = new HTTPTask(new Gson().toJson(callbackdata),
					"checkin", new WebServiceCallBack() {
						...
						public void onRequestComplete(Object responseData,
							...
							String data = (String) responseData;
							try {
								//CallBack objClass = new Gson().fromJson(data, CallBack.class);
								if (data.equals("0")) {
									
								}
								mServiceInterface.onComplete("");
								ISOSPlugin.CallBackCheckinService(data);
							} catch (Exception e) {
								e.printStackTrace();
								Toast.makeText(context, "Server Error",
										Toast.LENGTH_SHORT).show();
